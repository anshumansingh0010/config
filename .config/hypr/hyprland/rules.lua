...
tagged_rule(float_tag, {
    ...
    "Export Image as PNG",                 -- GIMP export
    "GIMP Crash Debug",                    -- GIMP crash debug
    "Save As",                             -- Save dialogs
    "Library",                             -- Library dialogs
}, "title")
...
hl.layer_rule({ match = { namespace = "caelestia-drawers" }, blur = true, ignore_alpha = 0.57 })

-- Tag-based misc rules (used with hyprbars and custom scripts)
hl.window_rule({ match = { tag = "noborder" }, border_size = 0 })
hl.window_rule({ match = { tag = "noshadow" }, no_shadow = true })
hl.window_rule({ match = { tag = "noblur" }, no_blur = true })

-- Persistent size (restore window size)
