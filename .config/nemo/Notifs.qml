...
Singleton {
    ...
    NotificationServer {
        ...
        onNotification: notif => {
            ...
            // Check if a notification with this replacement ID already exists
            const replId = notif.hints["x-c-repl-id"];
            const existingNotif = root.list.find(n => {
                // If the new notif has a replacement ID, check if any existing notif has the same one
                if (replId !== undefined) {
                    return n.notification.hints["x-c-repl-id"] == replId;
                }
                // Fallback to standard ID check (though strictly speaking we should rely on replace-id)
                return n.id == notif.id;
            });
            
            if (existingNotif) {
                // Update existing notification instead of creating a new one
                existingNotif.update(notif);
                existingNotif.time = new Date();
                
                // Only show popup again if it's NOT a progressive update (unless urgency changed to critical)
                // For progressive updates (like downloads), we usually don't want to spam popups
                // But let's keep it simple for now and just check DND/sidebar
                existingNotif.popup = !props.dnd && ![...Visibilities.screens.values()].some(v => v.sidebar);
                
                // Move to front of list
                root.list = [existingNotif, ...root.list.filter(n => n !== existingNotif)];
            } else {
